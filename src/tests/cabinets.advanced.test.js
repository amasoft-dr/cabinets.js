...
const blogStore = {
  ...
  operations: {
    ...
    removeComment: (state, articleId) => {
      state.blog.articles = state.blog.articles.filter(
  ...
  interceptors: {
    addArticle: (state, payload) => {
      localStorage.set("article_backups", {
        state,
        payload,
        lastUpdate: new Date()
      });
...
const blogStore2 = {
  ...
  interceptors: {
    def: (state, payload) => {
      localStorage.set("article_backups2", {
        ...state,
        payload,
        lastUpdate: new Date()
      });
...
it("Checks if default map was executed in Complex Store", () => {
  ...
  fire(actions.addArticle(article));
  const article1 = getState().blog.articles.find(
    (article) => article.author === "@andreidim"
  );
  ...
  const comment1 = getState().blog.comments.find(
    (comment) => comment.author === "@adpmaster"
  );
...
it("Checks if it's possible to combine stores", () => {
  ...
  const blogCounterStore = combineStores(
    "blogAndCounterStore",
    blogStateStore,
    counterStateStore
  );
  ...
  fire(actions.addArticle(article));
  const foundArticle = getState().blog.articles.find(
    (art) => art.author === "@eudys"
  );
...
it("Checks if subscribe/notify is working when state change...", (done) => {
  const { actions, fire, getState, subscribe, unsubscribe } = useStore(
    "blogAndCounterStore"
  );
  //state.counter is 20...
...
it("Checks if multiple subscribers are notified", (done) => {
  const { actions, fire, getState, subscribe, unsubscribe } = setupStore(
    basicCounterStore
  );
  //counter is 0...
  const subs = {
...
it("Checks if subscribe/notify is working wen state prop change...", (done) => {
  const { actions, fire, getState, subscribe } = useStore(
    "blogAndCounterStore"
  );
  //state.counter is 40...
...
it("Validatin Combining 2 stores", () => {
  ...
  const store1 = setupStore(commentsStore);
